"""
Phase 3 Training Script: MS-ASL Signer Diversity Fine-Tuning.
Loads Phase 2 checkpoint as the starting point and fine-tunes on MS-ASL100
...
from dataset_msasl import MSASLDataset, collate_fn
...
def save_checkpoint_phase3(model, out_dir):
    """Saves Phase 3 checkpoint to target directory."""
    ...
    print(f"Saved Phase 3 checkpoint to: {out_dir}")
...
def main():
    parser = argparse.ArgumentParser(description="ASL Signer Diversity Training (Phase 3 - MS-ASL100)")
    parser.add_argument("--config", default="config_phase3.yaml", help="Path to Phase 3 config file")
    ...
    # 6. LOAD MS-ASL100 DATASET
    msasl_cfg = cfg["msasl"]
    in_memory = not args.no_preload

    print("\nLoading MS-ASL100 training split...")
    train_ds = MSASLDataset(
        json_path=msasl_cfg.get("train_json"),
        landmarks_dir=msasl_cfg["landmarks_dir"],
        classes_path=msasl_cfg.get("classes_json"),
        synonym_path=msasl_cfg.get("synonym_json"),
        manifest_csv=msasl_cfg.get("manifest_csv"),
        split="train",
        max_classes=msasl_cfg.get("max_classes", 100),
        max_frames=cfg["max_frames"],
        in_memory=in_memory,
    )

    print("\nLoading MS-ASL100 validation split...")
    val_ds = MSASLDataset(
        json_path=msasl_cfg.get("val_json"),
        landmarks_dir=msasl_cfg["landmarks_dir"],
        classes_path=msasl_cfg.get("classes_json"),
        synonym_path=msasl_cfg.get("synonym_json"),
        manifest_csv=msasl_cfg.get("manifest_csv"),
    ...
    print(f"\nMS-ASL100 dataset splits: Train={len(train_ds)} | Val={len(val_ds)}")
    ...
    param_groups = model.trainable_parameter_groups(
        lr_projector=msasl_cfg["lr_projector"],
        lr_lora=msasl_cfg["lr_lora"],
    )
    ...
    if args.dry_run:
        ...
        print(f"Projector LR: {msasl_cfg['lr_projector']} | LoRA LR: {msasl_cfg['lr_lora']}")
    ...
    print(f"\nStarting Phase 3 training loop for {msasl_cfg['epochs']} epochs.")
    os.makedirs(out_dir, exist_ok=True)
    log_path = os.path.join(out_dir, "training_log_phase3.csv")
    log_rows = []
    best_val_loss = float("inf")

    for epoch in range(1, msasl_cfg["epochs"] + 1):
        train_loss = run_epoch(
            model, train_loader, optimizer, scaler, device, model.tokenizer,
            train=True, log_prefix=f"Phase3 Epoch {epoch}/[train]",
            grad_accum=grad_accum, max_grad_norm=max_grad_norm
        )

        val_loss = None
        if epoch % msasl_cfg["val_every"] == 0 and len(val_ds) > 0:
            with torch.no_grad():
                val_loss = run_epoch(
                    model, val_loader, optimizer, scaler, device, model.tokenizer,
                    train=False, log_prefix=f"Phase3 Epoch {epoch}/[val]",
                    grad_accum=grad_accum, max_grad_norm=max_grad_norm
                )
        ...
        # Save checkpoint
        save_checkpoint_phase3(model, out_dir)
        if val_loss is not None and val_loss < best_val_loss:
            best_val_loss = val_loss
            save_checkpoint_phase3(model, os.path.join(out_dir, "best"))
            print(f"New best Phase 3 checkpoint saved to: {out_dir}/best")

    print(f"\nPhase 3 training execution finished. Final checkpoints stored at: {out_dir}")
